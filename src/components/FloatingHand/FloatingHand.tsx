...
const SimpleCursorHand: React.FC<{
  gesture: HandGesture;
  size: number;
  dark: boolean;
}> = ({ gesture, size, dark }) => {
 ...
 */
const FloatingHandStandalone: React.FC<
    ...
    >
      ...
      {physics.shadowEnabled && (
        <div
          style={{
            position: 'absolute',
            left: finalX + physics.shadowDistance * 0.3,
            top: finalY + size * 0.8 + physics.shadowDistance,
            width: size * 0.5,
            height: size * 0.15,
            borderRadius: '50%',
            background: 'rgba(0, 0, 0, 0.15)',
            filter: `blur(${physics.shadowBlur * 0.8}px)`,
            transform: `translate(-50%, 0) scale(${finalScale * shadowScale})`,
            pointerEvents: 'none',
            zIndex: -1,
          }}
        />
