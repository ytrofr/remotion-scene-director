...
export function useHandAnimation(
  ...
  return useMemo(
    () => computeHandStateInner(relativeFrame, path, physics),
    [path, relativeFrame, physics],
  );
}
...
function computeHandStateInner(
  ...
  for (let i = 0; i < path.length; i++) {
    ...
    // Click gesture waypoints always get at least CLICK_ANIM_DURATION (45 frames)
    // so the full click animation plays. Saved data may have smaller values that
  ...
  for (let i = 0; i < timeline.length - 1; i++) {
    if (
      relativeFrame >= timeline[i].frame &&
      relativeFrame < timeline[i + 1].frame
    ) {
      segmentIndex = i;
      break;
    }
  ...
  const segmentProgress =
    segmentDuration > 0
      ? Math.min(
          1,
          Math.max(0, (relativeFrame - current.frame) / segmentDuration),
        )
      : 1;
  ...
  const velocityX =
    (deltaX / Math.max(1, segmentDuration)) * (1 + velocityMultiplier);
  const velocityY =
    (deltaY / Math.max(1, segmentDuration)) * (1 + velocityMultiplier);
  ...
  if (segmentProgress > 0.2 && segmentProgress < 0.8) {
    const pulseAmount =
      Math.sin(((segmentProgress - 0.2) / 0.6) * Math.PI) * 0.02;
    scale *= 1 + pulseAmount;
  }

  // Determine gesture with smooth transition timing
  // Click gestures activate late (at arrival), others transition early (at 30%)
  const clickThreshold = next.gesture === 'click' ? 0.9 : 0.3;
  const gesture =
    segmentProgress < clickThreshold ? current.gesture : next.gesture;
 ...
 */
export function useFloatEffect(physics: HandPhysicsConfig = DEFAULT_PHYSICS): {
  offsetY: number;
  shadowScale: number;
} {
  const frame = useCurrentFrame();
  ...
  return useMemo(() => {
    const offsetY =
      Math.sin(frame * physics.floatSpeed) * physics.floatAmplitude;
    const shadowScale = 1 + Math.sin(frame * physics.floatSpeed) * 0.1;
