...
// V1.10 — locked-rotation scrollbar physics (matches DorianDemo.scrollbar
// preset used in V1.09 scenes 2 + 8). Defined inline to avoid touching
// frozen DorianStores/constants.ts.
const SCROLLBAR_PHYSICS_V1_10 = {
  floatAmplitude: 0,
  floatSpeed: 0,
  velocityScale: 0,
  maxRotation: 0,
...
export const StoreDashboardSceneV1_10: React.FC = () => {
  ...
  // V1.10: Phase 9 — best-sellers scroll. BIG scrollbar drag at right edge
  // (X=880) Y 860→1060 over the bestSellerScroll window (frames 370-440).
  // Cursor is a separate FloatingHand instance, time-disjoint from handPath
  // (which fades at 225) and handPath3 (which starts at 480).
  const handPath2: HandPathPoint[] = [
    { x: 880, y: 860, frame: 360, gesture: 'pointer' as const, scale: 1 },
    { x: 880, y: 860, frame: 370, gesture: 'drag' as const, scale: 1 },
    { x: 880, y: 1060, frame: 440, gesture: 'drag' as const, scale: 1 },
    { x: 880, y: 1060, frame: 450, gesture: 'pointer' as const, scale: 1 },
  ];
  ...
  return (
    <AbsoluteFill style={{ background: COLORS.white }}>
      ...
      {/* V1.10 Hand cursor - phase 9 (best-sellers scrollbar drag) */}
      {frame >= 360 && frame < 460 && (
        <FloatingHand
          path={handPath2}
          startFrame={0}
          ...
          dark={false}
          showRipple={false}
          physics={SCROLLBAR_PHYSICS_V1_10}
        />
      )}
