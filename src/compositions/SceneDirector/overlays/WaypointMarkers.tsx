...
export const WaypointMarkers: React.FC<Props> = ({
  containerRef,
  editable = true,
  waypoints: waypointsProp,
}) => {
  const { state, dispatch, frame, currentScene, sceneWaypoints, composition } =
    useDirector();
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (!pos) return; // Container unmounted or zero-size — skip to prevent snap-to-origin
      // Subtract globalOffsetY: screen coords → scene-space coords
      dispatch({
        type: 'UPDATE_WAYPOINT',
        scene,
        index: dragging,
        point: { x: pos.x, y: pos.y - offsetY },
      });
    },
  ...
  if (currentScene && displayWaypoints.length >= 2) {
    ...
    const sortedWps = [...displayWaypoints].sort(
      (a, b) => (a.frame ?? 0) - (b.frame ?? 0),
    );
      ...
      >
        ...
        <g transform={offsetY ? `translate(0, ${offsetY})` : undefined}>
          ...
          {displayWaypoints.slice(1).map((wp, i) => {
            ...
            const angle =
              Math.atan2(wp.y - prev.y, wp.x - prev.x) * (180 / Math.PI);
            return (
              <g
                key={`arrow-${i}`}
            ...
            return (
              <g key={`wp-${i}`}>
                ...
                <circle
                  ...
                  fill={fill}
                  stroke={
                    isSelected || isDraggingThis ? '#fff' : 'rgba(0,0,0,0.5)'
                  }
                  strokeWidth={2}
          ...
          {displayWaypoints.map((wp, i) => {
            ...
            const prevGesture =
              i > 0 ? displayWaypoints[i - 1].gesture || 'pointer' : gesture;
